	...
	public static void getFileMD5(File[] files, String regex, boolean isCut) {
		...
		try {
			...
			for (File file : files) {
				...
				if (file.isFile()) {
					md5Map.put(CommonUtil.getFileDigest(file, "MD5"), file.getPath());
				} else {
					// 遍历文件夹下文件计算md5值
					...
					while (dirs.size() > 0) {
						...
						if (list != null && list.length > 0) {
							for (File f : list) {
								...
								if (f.isFile()) {
									String md5 = CommonUtil.getFileDigest(f, "MD5");
	...
	public static void checkFileExist(File dir, File info) {
		...
		try {
			...
			while((line = br.readLine()) != null) {
				...
				// 获取文件实际下载地址
				int uriStart = line.indexOf("http://filecloud.cc");
				int uriEnd = line.indexOf(filename, uriStart);
				String uri = line.substring(uriStart, uriEnd + filename.length()) + ".html"; 
